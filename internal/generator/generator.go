...
const (
	...
	pgsqlPkgName  = `pgsql`
	filterPkgPath = `github.com/frk/gosql/filter`
...
func buildQueryCode(g *generator, qs *analysis.QueryStruct) {
	...
	conn := GO.TypeNode(GO.QualifiedIdent{typ.PkgName, typ.Name})
	if typ.IsPtr {
		conn = GO.PointerType{conn}
	}
	...
	method.Name.Name = g.cfg.MethodName.Value
	method.Type.Params = GO.ParamList{{Names: GO.Ident{"c"}, Type: conn}}
	...
	if g.cfg.MethodWithContext.Value {
		ctxType := GO.TypeNode(GO.QualifiedIdent{"context", "Context"})
		method.Type.Params = GO.ParamList{
			{Names: GO.Ident{"ctx"}, Type: ctxType},
			method.Type.Params[0],
		}
	}
...
func buildQueryOutputInitRoot(g *generator, qs *analysis.QueryStruct) {
	...
	typeNode := GO.TypeNode(GO.Ident{typ.Base.Name})
	if typ.Base.IsImported {
		imp := addimport(g.file, typ.Base.PkgPath, typ.Base.PkgName)
		typeNode = GO.QualifiedIdent{imp.name, typ.Base.Name}
	}
	...
	for _, fr := range g.info.Reads {
		...
		for _, node := range fr.Field.Selector {
			...
			if node.IsPointer && !done[key] {
				typeNode := GO.TypeNode(GO.Ident{node.TypeName})
				if node.IsImported {
					imp := addimport(g.file, node.TypePkgPath, node.TypePkgName)
					typeNode = GO.QualifiedIdent{imp.name, node.TypeName}
				}
...
func addConverterCallExpr(g *generator, x GO.ExprNode, converter string) GO.ExprNode {
	if len(converter) > 0 {
		addimport(g.file, pgsqlPkgPath, pgsqlPkgName)
		sel := GO.SelectorExpr{X: GO.Ident{pgsqlPkgName}, Sel: GO.Ident{converter}}
...
func buildFilterColumnMap(g *generator, fs *analysis.FilterStruct) (mapid GO.Ident) {
	...
	for _, filter := range g.info.Filters {
		...
		if fs.IsV2() {
			felems := []GO.FieldElement{{Field: "Name", Value: GO.RawStringLit(g.fmtColIdent(filter.ColIdent))}}
			if id := filter.Field.FilterValueConverter(); id != nil {
				fe := GO.FieldElement{Field: "ConvertValue"}
				if id.NeedsImport(g.info.Info) {
					imp := addimport(g.file, id.PkgPath, "")
					fe.Value = GO.QualifiedIdent{imp.name, id.Name}
				} else {
					fe.Value = GO.Ident{id.Name}
				}
	...
	if fs.IsV2() {
		imp := addimport(g.file, filterPkgPath, filterPkgName)
		maplit.Type = GO.MapType{Key: GO.Ident{"string"}, Value: GO.QualifiedIdent{imp.name, "Column"}}
	}
	maplit.Elems = elems
...
func buildQueryCallExec(g *generator, qs *analysis.QueryStruct) {
	...
	if g.cfg.MethodWithContext.Value || qs.Context != nil {
...
func newImportDecl(f *file) *GO.ImportDecl {
	...
	for _, s := range imports.Specs {
		if strings.HasPrefix(string(s.Path), `github.com/frk/gosql`) {
...
func makeColIdentFormatter(cfg config.Config) (fn func(analysis.ColIdent) string) {
	if cfg.QuoteIdentifiers.Value {
