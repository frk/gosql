...
func Write(f io.Writer, pkgName string, targets []*postgres.TargetInfo, conf Config) error {
	...
	conn := defaultConnType
	if len(conf.ConnType.Name) > 0 {
		conn = conf.ConnType
	}
...
type generator struct {
	...
	file *file
	// Info on the type of the Conn argument to a to-be generated Exec method.
	conn ConnType
...
func buildQueryCode(g *generator, qs *analysis.QueryStruct) {
	imp := addimport(g.file, g.conn.PkgPath, g.conn.PkgName)
	g.conn.PkgName = imp.name
	...
	conn := GO.TypeNode(GO.QualifiedIdent{g.conn.PkgName, g.conn.Name})
	if g.conn.IsPtr {
