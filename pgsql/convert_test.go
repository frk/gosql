...
func (list testlist2) execute(t *testing.T, coltype string) {
	for _, tt := range list {
		for _, td := range tt.data {
			dest := reflect.New(reflect.TypeOf(td.output))

			// resolve valuer
			var valuer interface{}
			if tt.valuer != nil {
				rvals := []reflect.Value{reflect.ValueOf(td.input)}
				valuer = reflect.ValueOf(tt.valuer).Call(rvals)[0].Interface()
			} else {
				valuer = td.input
			}
			...
			if tt.scanner != nil {
				rvals := []reflect.Value{dest}
				scanner = reflect.ValueOf(tt.scanner).Call(rvals)[0].Interface()
			} else {
				scanner = dest.Interface()
			}
